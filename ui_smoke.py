...
game.load_catalog(catalog.TOYS, catalog.FOODS, cats, catalog.CATEGORIES, catalog.GOODS, catalog.ACTORS)
...
game.load_catalog(catalog.TOYS, catalog.FOODS, catalog.CATS, catalog.CATEGORIES, catalog.GOODS, catalog.ACTORS)
...
game.load_catalog(catalog.TOYS, catalog.FOODS, catalog.CATS + [("longcat", "ながいねこ", long_desc, "お" * 60)], catalog.CATEGORIES, catalog.GOODS, catalog.ACTORS)
...
app.shop_kind = None
game.load_catalog(catalog.TOYS, catalog.FOODS, catalog.CATS, catalog.CATEGORIES, catalog.GOODS, catalog.ACTORS)
app.state["met_order"] = [m for m in app.state["met_order"] if m in game.CATS]     # 1000匹テストの残骸を掃除
app.state["cats"] = {k: v for k, v in app.state["cats"].items() if k in game.CATS}

# ------------------------------------------------------------ ずかん(ひと)・所持品からの「あげる」(フェーズ3)
app.state["level"] = 5
game._tick_actors(app.state, random.Random(0), [])         # キャツアイが現れ、スマホをくれる。じろうさんも同時に現れる
assert "catseye_a" in app.state["met_actors"] and "jiro" in app.state["met_actors"]
assert "smartphone" in app.state["owned_goods"]

tab("cats")
click(8 + 32 + 68, 28); assert app.sub["cats"] == 1         # ひと のサブタブ
people = [aid for aid in game.ACTORS if aid in app.state["met_actors"]]
click(30, row_y(people.index("jiro"), CATS_TOP))
assert app.selected["people"] == "jiro"
settle(); frame(); shot("18_people")
assert app.person_pager.pages and app.person_pager.key[1] == "jiro"
click(8 + 32, 28); assert app.sub["cats"] == 0              # 猫のサブタブに戻しておく

tab("bag")
click(8 + 32 + 68 * 2, 28); assert app.sub["bag"] == 2      # 所持品のサブタブ
goods = sorted(app.state["owned_goods"])
click(30, row_y(goods.index("smartphone"), 40))
assert app.give_picker == "smartphone"
settle(); frame(); shot("19_give_picker")
click(128, 102)                                             # あげる相手(じろうさん、1人だけ)をタップ
assert app.give_picker is None
assert app.state["pets"] == {"turtle"} and "unlocked_obaachan" in app.state["flags"]
assert "smartphone" not in app.state["owned_goods"]
click(8 + 32 + 68 * 2, 28); assert app.sub["bag"] is None   # もう一度押して解除しておく
